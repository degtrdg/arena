...
def create_comparison_data(df):
    ...
    for context in sorted(specific_df['eval_context'].unique()):
        ...
        if context in general_df.index:
            ...
            # If there is no general evaluation for a specific context, we can't compare.
            # This can happen if the general vector was not evaluated on all specific contexts.
            # We will skip this context.
            continue
...
def main():
    ...
    files = glob.glob('experiment1/eval_results/persuasion/*.json')
    raw_data = []
    for f in files:
    ...
    persuasion_df = df[df['eval_context'].isin(persuasion_contexts)]

    # --- Generate Persuasion Graphs ---
    persuasion_comp_df = create_comparison_data(persuasion_df)
    plot_single_score_type(persuasion_comp_df, "persuasion", "Positive")
    plot_single_score_type(persuasion_comp_df, "persuasion", "Negative")
