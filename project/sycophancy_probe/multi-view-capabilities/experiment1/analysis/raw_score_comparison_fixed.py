...
def parse_filename(filename):
    ...
    if 'sycophancy' in base_name and 'probe_weights' not in base_name:
        return 'general', 'general'
    if 'pop_culture_bipo' in base_name:
        return 'specific', 'pop_culture_opinion_alignment'
    return None, None
...
def create_comparison_data(df):
    ...
    if general_df.empty:
        print("General dataframe is empty")
        return pd.DataFrame()
    
    if specific_df.empty:
        print("Specific dataframe is empty")
        return pd.DataFrame()
        ...
        for item in data:
            if 'context_label' in item:
                raw_data.append({
                    ...
                    'positive_steering_score': item.get('positive_steered_score'), 'negative_steering_score': item.get('negative_steered_score'),
                })
    
    df = pd.DataFrame(raw_data)
    for col in ['positive_steering_score', 'negative_steering_score']:
        df[col] = pd.to_numeric(df[col], errors='coerce').fillna(0)

    df = df.groupby(['vector_type', 'vector_context', 'eval_context']).mean().reset_index()
    
    sycophancy_contexts = ['pop_culture_opinion_alignment', 'fashion_choice_affirmation', 'parenting_style_approval', 'political_ideology_alignment']

    sycophancy_df = df[df['eval_context'].isin(sycophancy_contexts)]

    # --- Generate Sycophancy Graphs ---
    sycophancy_comp_df = create_comparison_data(sycophancy_df)
    plot_single_score_type(sycophancy_comp_df, "sycophancy", "Positive")
    plot_single_score_type(sycophancy_comp_df, "sycophancy", "Negative")
