...
def main():
    ...
    files = glob.glob('experiment1/eval_results/pessimism/*.json')
    raw_data = []
    for f in files:
    ...
    pessimism_df = df[df['eval_context'].isin(pessimism_contexts)]

    # --- Generate Pessimism Graphs ---
    pessimism_comp_df = create_comparison_data(pessimism_df)
    plot_single_score_type(pessimism_comp_df, "pessimism", "Positive")
    plot_single_score_type(pessimism_comp_df, "pessimism", "Negative")
