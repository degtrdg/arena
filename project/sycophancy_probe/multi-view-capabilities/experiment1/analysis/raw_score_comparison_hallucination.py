...
def main():
    ...
    files = glob.glob('experiment1/eval_results/hallucination/*.json')
    raw_data = []
    for f in files:
    ...
    hallucination_df = df[df['eval_context'].isin(hallucination_contexts)]

    # --- Generate Hallucination Graphs ---
    hallucination_comp_df = create_comparison_data(hallucination_df)
    plot_single_score_type(hallucination_comp_df, "hallucination", "Positive")
    plot_single_score_type(hallucination_comp_df, "hallucination", "Negative")
